      SUBROUTINE RFFORM(STR,LENIN,A,ISTP,NUM,IERR)
C
...
C Character "!" might have been used on an input line to allow for a COMMENT
      LEN= INDEX(STR,'!') - 1
      IF( LEN .LT. 0 )   LEN= LENIN
  ...
  12     IP=IP+1
     ...
     2            STR(IP-1:IP-1).NE.' ')) THEN
           ...
           CALL CONSSR(BUF,LBUF,A((I-1)*ISTP+1),IERR)
         ELSE
           GO TO 12
         END IF
         IP=IP-1
 ...
 10    CONTINUE
      RETURN
      END
      SUBROUTINE CONSSR(BUF,LBUF,A,IERR)
      CHARACTER*(*) BUF
      LOGICAL DEC,SIG,CIF
      A=0
      IL=ICHAR('0')
      ...
      ELSE IF (BUF(I:I).EQ.'E') THEN
        CALL CONSSI(BUF(I+1:LBUF),LBUF-I,IE,IERR)
        IDEC=MIN0(IDEC,I-1)
 ...
 10   CONTINUE
      ...
      RETURN
      END
      SUBROUTINE CONSSI(BUF,LBUF,IA,IERR)
      CHARACTER*(*) BUF
      LOGICAL SIG,CIF
      IA=0
      IL=ICHAR('0')
